...
set_default_stored_searches() {
	...
	local cidr_search="$(get_tmux_option "$copycat_cidr_search_option" "$default_cidr_search_key")"
	...
	if stored_search_not_defined "$cidr_search"; then
		tmux set-option -g "${COPYCAT_VAR_PREFIX}_${cidr_search}" "[[:digit:]]{1,3}\.[[:digit:]]{1,3}\.[[:digit:]]{1,3}\.[[:digit:]]{1,3}/[[:digit:]]{1,2}"
	fi
}
